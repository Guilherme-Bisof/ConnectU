...
import { FiMessageSquare, FiUser, FiBriefcase, FiSearch } from "react-icons/fi";
...
export default function ChatPage() {
  ...
  const [filteredConversations, setFilteredConversation] = useState<Room[]>([]);
  ...
  <div className="flex h-[calc(100vh-4rem)] w-full bg-zinc-950 text-white overflow-hidden">
        ...
        <div className="flex gap-2">
          {(["TODOS", "STUDENT", "RECRUITER"] as const).map((filter) => (
            <button
              ...
              className={`text-xs px-3 py-1.5 rounded-full font-medium border transition-all ${activeFilter === filter ? "bg-blue-600 border-blue-500 text-white shadow-md shadow-blue-600/10" : "bg-zinc-950 border-zinc-800 text-zinc-400 hover:border-zinc-700 hover:text-zinc-200"}`}
            >
              {" "}
              {filter === "TODOS"
                ? "Todos"
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center justify-between gap-2 mb-0.5">
                    ...
                    <span
                      className={`text-[10px] uppercase tracking-wider px-1.5 py-0.5 rounded shrink-0 font-bold ${otherUser.role === "RECRUITER" ? "bg-purple-500/10 text-purple-400 border border-purple-500/20" : "bg-blue-500/10 text-blue-400 border border-blue-500/20"}`}
                    >
                      {otherUser.role === "RECRUITER" ? "Recrutador" : "Aluno"}
                    </span>
                  </div>
    ...
    <main className="hidden md:flex flex-1 flex-col items-center justify-center bg-zinc-950/40 p-8 text-center border-l border-zinc-900">
        <div className="w-16 h-1/6 rounded-full bg-zinc-900 border border-zinc-800 flex items-center justify-center text-zinc-600 mb-4 shadow-xl">
            <FiMessageSquare size={28} />
        </div>
        <h3 className="text-base font-bold text-zinc-300">Sua Central de Mensagens</h3>
        <p className="text-xs text-zinc-500 max-w-xs mt-1 leading-relaxed">
            Selecione uma conversa na lista lateral para abrir o chat em tempo real para entrar em contato com amigos ou negociar oportunidades 
        </p>
    </main>
  </div>;
}
